...
- **Python database integration:** the local targeted run safely skipped all 7 because Docker/PostGIS is unavailable and the only locally configured URL is not an authorized destructive test target. GitHub Actions run `32090657115` then injected its isolated PostGIS URL explicitly: 7 executed, 7 passed, 0 failed, 0 skipped. The normal local Python suite passed 39 tests and skipped the 7 guarded integration tests as designed.
...
- **Final issue status:** RESOLVED. The credential rotation, local read-only connection, web integration runner, production deployment, and isolated Python integration runner all passed. Automatic loading of the production credential into destructive Python tests remains intentionally prohibited.

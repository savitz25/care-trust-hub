...
export function stateCareRecovery(query: SeniorResearchQuery) {
  ...
  const routes: Record<string, { href: string; label: string }> = {
    ...
    NC: { href: "/north-carolina", label: "Open North Carolina DHSR care-setting research" },
    ...
    NV: { href: "/nevada", label: "Open Nevada HCQC care-setting research" },
    MN: { href: "/minnesota", label: "Open Minnesota MDH care-setting research" },
  };
  return state
    ? (routes[state] ?? null)

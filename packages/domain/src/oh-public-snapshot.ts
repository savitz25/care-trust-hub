/** Generated by scripts/build-oh-public-snapshot.py. Do not edit by hand. */
export const OH_PUBLIC_SNAPSHOT = {
  version: "senior-oh-state-intel-v1",
  ticket: "OH-SEN-001",
  publicationPath: "/ohio",
  asOf: "2026-09-18",
  snapshotAsOf: "2026-09-18",
  retrievedAt: "2026-09-18T13:26:20Z",
  generatedAt: "2026-09-18T13:46:04Z",
  localWorkNeededNow: "NO",
  claimEligibilityBroadened: false,
  no_trust_score: true,
  no_aggregate_rating: true,
  no_rankings: true,
  no_cleveland_page: true,
  no_columbus_page: true,
  clocks: {
    generatedAt: "2026-09-18T13:46:04Z",
    snapshotAsOf: "2026-09-18",
    retrievedAt: "2026-09-18T13:26:20Z",
    nh_onesource_sourceAsOf: "2026-08-10",
    rcf_onesource_sourceAsOf: "2026-06-17",
    cms_sourceAsOf: "national-intelligence-overlay",
    navigator_sourceAsOf: null,
    odh_license_source_date_nh: "2026-08-10",
    odh_license_source_date_rcf: "2026-06-17",
    inspection_date: null,
    satisfaction_survey_period: null,
    quality_measure_period: null,
    home_health_license_date: null,
    hospice_license_date: null,
    pace_source_date: null,
    retrievedAt_is_not_sourceAsOf: true,
  },
  regulatorMap: {
    odh: "Ohio Department of Health",
    age: "Ohio Department of Aging",
    navigator: "https://aging.ohio.gov/care-and-living/ohio-aging-compass/ohio-aging-compass-home",
    odhExtract: "https://publicapps.odh.ohio.gov/eid/Default.aspx",
    cmsCareCompare: "https://www.medicare.gov/care-compare/",
  },
  nursingHomes: {
    OH_NURSING_FACILITY_ROSTER_STATUS: "ACQUIRED_CURRENT_SNAPSHOT",
    OH_NURSING_FACILITY_ROWS: 923,
    OH_NURSING_FACILITY_DISTINCT_STATE_IDS: 923,
    OH_NURSING_FACILITY_DISTINCT_CCNS: null,
    source: "ODH OneSource ArcGIS Nursing Homes Licensed and Certified in Ohio",
    sourceAsOf: "2026-08-10",
    licensePrefix: "OH",
    all_active: true,
    caveat:
      "ODH license is not a CMS CCN. 923 ODH rows vs 922 CMS Ohio nursing homes is not an identity bridge.",
  },
  rcf: {
    OH_RCF_ROSTER_STATUS: "ACQUIRED_CURRENT_SNAPSHOT",
    OH_RCF_ROWS: 812,
    OH_RCF_DISTINCT_STATE_LICENSE_IDS: 812,
    OH_HOME_FOR_AGING_COMPONENT_ROWS: null,
    consumerLanguage: "assisted living",
    stateNativeClass: "Residential Care Facility",
    source: "ODH OneSource ArcGIS Assisted Living / Residential Care Facilities in Ohio",
    sourceAsOf: "2026-06-17",
    licensePrefix: "OHL",
    all_active: true,
    caveat:
      "RCF is Ohio's formal assisted-living license class. It is not a Nursing Home and not a Medicaid waiver universe.",
  },
  navigator: {
    OH_NAVIGATOR_NURSING_ROWS: null,
    OH_NAVIGATOR_RCF_ROWS: null,
    coverage: "OPEN_SEARCH_ONLY",
    reason:
      "The public Long-Term Care Quality Navigator is a dashboard (Aging Compass). aging.ohio.gov/Navigator currently 404s. Structured Navigator JSON/API was not acquired. Official ODH OneSource license layers are the freeze for facility identity.",
    not_trusthub_rating: true,
    quality_measure_ne_trusthub: true,
    satisfaction_ne_aggregaterating: true,
  },
  inspections: {
    OH_NURSING_INSPECTION_ROWS: null,
    OH_RCF_INSPECTION_ROWS: null,
    OH_NURSING_DEFICIENCY_ROWS: null,
    OH_RCF_VIOLATION_ROWS: null,
    coverage: "OPEN_SEARCH_ONLY",
    inspection_ne_complaint: true,
  },
  complaints: {
    OH_NURSING_HOME_COMPLAINT_ROWS: null,
    OH_RCF_COMPLAINT_ROWS: null,
    coverage: "INTAKE_AVAILABLE / BULK_NOT_PUBLIC",
  },
  quality: {
    OH_NURSING_QUALITY_MEASURE_OBSERVATIONS: null,
    OH_RCF_SATISFACTION_OBSERVATIONS: null,
    coverage: "OPEN_SEARCH_ONLY",
  },
  homeHealth: {
    OH_SKILLED_HOME_HEALTH_AGENCY_ROWS: null,
    OH_NONMEDICAL_HOME_HEALTH_AGENCY_ROWS: null,
    OH_SKILLED_NONAGENCY_PROVIDER_ROWS: null,
    OH_NONMEDICAL_NONAGENCY_PROVIDER_ROWS: null,
    OH_HOME_HEALTH_DISTINCT_STATE_LICENSE_IDS: null,
    coverage: "OPEN_SEARCH_ONLY",
    path: "ODH Health Care Provider Report and Information Extract (FACILITY LISTING) is interactive; statewide bulk extract was not frozen in this ticket.",
    skilled_ne_nonmedical: true,
    agency_ne_nonagency: true,
    state_license_ne_cms: true,
  },
  hospice: {
    OH_HOSPICE_ROWS: null,
    OH_HOSPICE_DISTINCT_STATE_LICENSE_IDS: null,
    OH_HOSPICE_LOCATION_ROWS: null,
    coverage: "OPEN_SEARCH_ONLY",
    license_ne_location: true,
  },
  cmsOverlay: {
    nursingHomes: 922,
    homeHealth: 835,
    hospice: 169,
    CMS_OH_NURSING_HOME_ROWS: 922,
    CMS_OH_HOME_HEALTH_ROWS: 835,
    CMS_OH_HOSPICE_ROWS: 169,
    do_not_add_to_state: true,
    addedToNationalTotals: false,
    stateLicense_ne_cms: true,
  },
  crosswalk: {
    EXACT_OH_STATE_TO_CMS_NURSING_HOME_BRIDGES: 0,
    EXACT_OH_STATE_TO_CMS_HOME_HEALTH_BRIDGES: 0,
    EXACT_OH_STATE_TO_CMS_HOSPICE_BRIDGES: 0,
    EXACT_OH_NAVIGATOR_TO_ODH_FACILITY_BRIDGES: 0,
    OH_HOSPICE_DISTINCT_CCNS: null,
    name_only: "UNSAFE",
    reason: "OneSource license layers do not publish CCN. 923 vs 922 is not a bridge.",
  },
  programs: {
    OH_PACE_ROSTER_STATUS: "OPEN_SEARCH_ONLY",
    OH_PACE_ORGANIZATION_ROWS: null,
    OH_PACE_LOCATION_ROWS: null,
    OH_ASSISTED_LIVING_WAIVER_PROVIDER_ROWS: null,
    OH_ADULT_DAY_ROSTER_STATUS: "OPEN_SEARCH_ONLY",
    OH_ADULT_DAY_ROWS: null,
    waiver_ne_rcf_license: true,
    pace_ne_facility_license: true,
  },
  ownership: {
    structured_owner_field_on_onesource: false,
    ownership_ne_facility_identity: true,
  },
  expansion_ledger: {
    NET_NEW_STATE_RESEARCH_IDENTITIES: 1735,
    NET_NEW_CANONICAL_FACILITIES: 0,
    NET_NEW_PUBLIC_PROFILES: 0,
    EXISTING_FACILITIES_ENRICHED: 0,
    GRAPH_WRITES: 0,
    CLAIM_ELIGIBILITY_BROADENED: false,
  },
  adverse_publication: {
    ADVERSE_SOURCES_FOUND: 2,
    ADVERSE_SOURCES_ACQUIRED: 0,
    ADVERSE_ROWS_ACQUIRED: null,
    UNIQUE_REGULATORY_MATTERS: null,
    EXACT_PROFILE_ATTACHMENTS: 0,
    REVIEW_REQUIRED: 0,
    UNRESOLVED: null,
    INTERNAL_ONLY: 0,
    PUBLICATION_PENDING: 0,
    PUBLIC_READY_PROFILES: 0,
    PUBLICLY_RENDERED_PROFILES: 0,
    BUSINESS_RESPONSE_READY: false,
    SEARCH_SUPPORTED: true,
    REMAINING_ADVERSE_GAPS: [
      "Navigator quality-measure grains",
      "Navigator consumer-satisfaction grains",
      "ODH nursing inspection events",
      "ODH RCF inspection events",
      "nursing deficiency / SOD bulk",
      "RCF violation bulk",
      "complaint bulk",
    ],
    WITHHELD_REASON_COUNTS: {
      OPEN_SEARCH_ONLY: 7,
      "INTAKE_AVAILABLE / BULK_NOT_PUBLIC": 1,
      do_not_sum_inspection_complaint_deficiency_enforcement: true,
    },
  },
  licenseIdentityNote:
    "Nursing Home licenses use OH#####. RCF licenses use OHL#####. Zero license-number overlap. Shared business names (48) are not identity bridges.",
  fingerprint: "f4e8f7706cf758403dc7e11883c936a14d080fd24d3fc25dca8ae3031c7c5578",
} as const;
export type OhPublicSnapshot = typeof OH_PUBLIC_SNAPSHOT;
